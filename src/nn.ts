...
export class Activations {
  ...
  public static SOFTMAX: ActivationFunction = {
    output: x => x,
    der: x => 1
  }
}
...
export function forwardProp(network: Node[][], inputs: number[]): number[] {
  ...
  //softmax on output layer only if output layer is bigger than 1
  const outputLayer = network[network.length-1]; 
  if (outputLayer.length > 1) {
    const expValues = outputLayer.map(n => Math.exp(n.totalInput));
    const sumExp = expValues.reduce((a,b) => a + b, 0);
    outputLayer.forEach((n,i) => { n.output = expValues[i] / sumExp; });
  }

  return outputLayer.map(n => n.output);
}
